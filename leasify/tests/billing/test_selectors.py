...
from leasify.billing import selectors as sl
...
class TestBillingListFor:
    ...
    def test_filtering_based_on_query_params(self, manager_user, user_factory, billing_factory, tenancy_factory, today):
        ...
        billing_factory(tenancy=t[0], statuses=[BS.PAID, BS.CANCELLED, BS.UNPAID], starting=date(2024, 1, 1))
        billing_factory(tenancy=t[1], statuses=[BS.CANCELLED, BS.PAID], starting=date(2024, 2, 1))
...
def test_billing_get_for_role_based_filtering(
    superuser: User,
    manager_user: User,
    caretaker_user: User,
    tenancy_factory: Factory[Tenancy],
...
def test_billing_last_paid_for(tenancy_factory: Factory[Tenancy], billing_factory: Factory[BP]):
    ...
    billing_factory(tenancy=t[1], statuses=[BS.CANCELLED, BS.CANCELLED, BS.UNPAID])
