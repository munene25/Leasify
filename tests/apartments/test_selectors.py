...
class TestApartmentList:
    ...
    def test_apartment_for_regular_user(self, user_factory, apartment_factory, ):
        """Test how Policy filters based on user"""
        apartments = apartment_factory(5)
        ...
        tenancy_create(user=users[0], apartment=apartments[0], start_date=last_month.start_date, duration_months=2)
        
        
        # assert False
        list_2_for_user1 = apartment_list_for(user=users[0], filters={})
        assert list_2_for_user1.count() == 5
